...
% Sprint 3 - Schedule Rooms
find_available_room(Day, Room) :-
    operation_room(Room),
    agenda_operation_room(Room, Day, _),
    \+ agenda_operation_room1(Room, Day, _).

schedule_all_surgeries_heuristic([], _, _).
schedule_all_surgeries_heuristic([OpCode | Rest], _, Day) :-
    surgery_id(OpCode, OpType),
    surgery(OpType, _, TSurgery, _),

    % Encontrar a sala disponível mais cedo usando a heurística
    find_available_room(Day, Room),
    availability_operation(OpCode, Room, Day, LPossibilities, _),
    schedule_first_interval(TSurgery, LPossibilities, (TinS, TfinS)),

    % Atualizar agendas dos médicos
    findall(Doctor, assignment_surgery(OpCode, Doctor), LDoctors),
    insert_agenda_doctors((TinS, TfinS, OpCode), Day, LDoctors),
    ...
    % Imprimir atribuição da sala
    format('Surgery ~w assigned to room ~w from ~w to ~w~n', [OpCode, Room, TinS, TfinS]),

    % Continuar com a próxima cirurgia
    schedule_all_surgeries_heuristic(Rest, _, Day).

schedule_all_surgeries :-
    findall(OpCode, surgery_id(OpCode, _), Cirurgias),
    Dia = 20241028, % Definir o dia desejado aqui
    schedule_all_surgeries_heuristic(Cirurgias, _, Dia).
