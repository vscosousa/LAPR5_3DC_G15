...
% Predicate to get staff agendas
list_staff_agenda(_, [], []).
list_staff_agenda(Day, [Staff | LStaff], [(Staff, Agenda) | LAgendas]) :-
...
obtain_heuristic_sol(Room, Day, AgOpRoomBetter, LAgDoctorsBetter, TFinOp) :-
    ...
    % Get anaesthetists schedules
    %findall(Anaesthetist, staff(Anaesthetist, anaesthetist, _, _), Anaesthetists),
    %list_staff_agenda(Day, Anaesthetists, AnaesthetistsAgendas),
    ...
    % format('~nAnaesthetist Schedules: ~w~n', [AnaesthetistsAgendas]),
