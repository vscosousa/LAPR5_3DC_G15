    ...
    public class UserServiceTests
    {
      ...
      /*  [Fact]
        ...
        public async Task RequestDeletePatientUser_Successfully()
        {
            ...
            var user = new User(patient.Email, "PasswordCorrect123@", patient.PhoneNumber, patient.Id.AsGuid());
            var token = _userService.CreateToken(user);
            var name = patient.Email;

            _userRepositoryMock.Setup(repo => repo.GetByIdAsync(user.Id)).ReturnsAsync(user);
            _mailServiceMock.Setup(mail => mail.SendDeletePatientUserEmailAsync(user.Email,name, It.IsAny<string>())).Returns(Task.CompletedTask);
            ...
            _mailServiceMock.Verify(mail => mail.SendDeletePatientUserEmailAsync(user.Email, name, It.IsAny<string>()), Times.Once);
        }
